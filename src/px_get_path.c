...
/*   Updated: 2025/08/20 12:33:52 by kchiang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "pipex.h"

static int	px_path_is_valid(char *cmd_path);
...
char	*px_get_path(char **cmd, char **envp)
{
	...
	if (cmd[0] && px_path_is_valid(cmd[0]))
...
static int	px_path_is_valid(char *cmd_path)
{
	if (access(cmd_path, F_OK) == 0)
	{
		...
		else
		{
			...
			px_perror_exit(cmd_path, PERM_DENIED);
		}
	}
	return (false);
}
